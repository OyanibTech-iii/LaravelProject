                    ...
                    <form action="{{ route('products.update', $product->id) }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="stock_quantity" name="stock_quantity" type="number" class="mt-1 block w-full" :value="$product->stock_quantity" required />
                                <x-input-error class="mt-2" :messages="$errors->get('stock_quantity')" />
                            </div>

                            <div class="col-span-2">
                                <x-input-label for="image_path" :value="__('Image Filename')" />
                                <x-text-input id="image_path" name="image_path" type="text" class="mt-1 block w-full" :value="$product->image_path" placeholder="e.g., assets/images/Matcha.jfif" />
                                <x-input-error class="mt-2" :messages="$errors->get('image_path')" />
                            </div>

                            <div class="col-span-2">
                                <x-input-label for="description" :value="__('Description')" />
                                <textarea id="description" name="description" class="mt-1 block w-full bg-gray-50/50 border-gray-200 focus:border-brick focus:ring-brick rounded-xl shadow-sm transition-all duration-200 text-sm" rows="3">{{ $product->description }}</textarea>
